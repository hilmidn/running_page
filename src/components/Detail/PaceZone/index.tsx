...
import { createPaceZoneData } from '@/utils/activityAnalytics';
...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-lg bg-gray-900/95 px-3 py-2 text-xs text-white shadow-lg backdrop-blur-sm">
      ...
      <div className="text-gray-400">{d.paceRange}</div>
      <div className="text-gray-400">Avg {d.formattedAvgPace}</div>
    </div>
  );
}

export default function PaceZoneChart({ stream, compact }: Props) {
  const totalTime = stream.time?.[stream.time.length - 1] ?? 0;

  const zoneData = useMemo(
    () => createPaceZoneData(stream, totalTime),
    [stream, totalTime],
  );

  const hasData = stream.distance?.length > 1;
  const hasZones = zoneData.some((z) => z.percentage > 0);
  ...
  if (!hasZones) {
    return (
      <div className="rounded-xl bg-gray-800/40 p-6 text-center">
        ...
        <p className="text-sm text-gray-400">No pace zone data available</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl bg-gray-800/30 p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-gray-200">Pace Zones</h3>
      </div>
          ...
          .map((z) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-500">{z.paceRange}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-gray-400">{z.formattedAvgPace} avg</span>
